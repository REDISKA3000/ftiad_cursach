...
class VBenchMetric:
    ...
    def run(self, video_path, dimensions=None, extra_context=None):
        ...
        # Sequential mode: one dim at a time with memory flush between each
        if len(requested_dimensions) > 1:
            return self._run_sequential(video_path, requested_dimensions, extra_context)

        return self._run_single(video_path, requested_dimensions, extra_context)

    def _run_sequential(self, video_path, dimensions, extra_context=None):
        """Run each VBench dimension separately, clearing GPU memory between each."""
        import gc
        merged_flat: dict = {}
        merged_raw: dict = {}
        failed_dims: list = []
        ...
        for dim in dimensions:
            result = self._run_single(video_path, [dim], extra_context)
            if result["status"] != "failed":
                merged_flat.update(result["flat_scores"])
                merged_raw.update(result["details"].get("raw_result") or {})
            else:
                failed_dims.append(dim)
            # Free GPU memory after each dimension
            try:
                import torch
                gc.collect()
                torch.cuda.empty_cache()
                # Reset backend so VBench re-initialises its model registry next dim
                self._backend = None
                self._backend_output_dir = None
            except Exception:
                pass

        missing = [d for d in dimensions if d not in merged_flat]
        status = "ok" if not missing else ("partial" if merged_flat else "failed")
        error = None
        if failed_dims:
            error = f"Failed dims: {', '.join(failed_dims)}"

        parsed = parse_vbench_result(merged_raw, dimensions=dimensions)
        parsed["flat_scores"] = merged_flat
    ...
    def _run_single(self, video_path, dimensions, extra_context=None):
        """Run VBench for a list of dimensions in one backend.evaluate() call."""
        ...
        try:
            ...
            if verbose:
                ...
                print(f"[VBench] dims: {dimensions}")
            ...
            parsed = parse_vbench_result(raw_result, dimensions=dimensions)
